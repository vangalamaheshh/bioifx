...
ANALYSIS_DIR="/media/mvangala/pig_rnseq_data/SNP"
#ANALYSIS_DIR="/media/mvangala/mbcf_storage_aberdeen/SNP/analysis/Sun_Jun_21_00_03_58_EDT_2015"
REF_FASTA="/media/mvangala/mbcf_storage_aberdeen/ref_files/pig/UCSC/Sus_scrofa/UCSC/susScr3/Sequence/WholeGenomeFasta/genome.fa"
STAR_GENOME_INDEX_DIR="/media/mvangala/mbcf_storage_aberdeen/ref_files/pig/UCSC/Sus_scrofa/UCSC/susScr3/Sequence/STARIndex"
...
function align_reads {
	...
	local index_dir=$3
	mkdir -p $align_dir
	left_file=$(find $trim_dir -type f -name "*.trimmomatic.left_paired.fastq")
	right_file=$(find $trim_dir -type f -name "*.trimmomatic.right_paired.fastq")
	cd $align_dir
...
function process_picard {
	...
	#java -jar /media/mvangala/data/software/bioinfo/format/picard-tools-1.115/AddOrReplaceReadGroups.jar I="${two_pass_dir}/${sample_name}.Aligned.out.sam" \
		O="${picard_dir}/${sample_name}.rg_added_sorted.bam" SO=coordinate RGID=id RGLB=library RGPL=platform RGPU=machine RGSM=sample

	#Mark duplicates
	java -jar /media/mvangala/data/software/bioinfo/format/picard-tools-1.115/MarkDuplicates.jar I="/media/mvangala/pig_rnseq_data/sorted_bam/${sample_name}.sorted.bam" \
...
for left_file in $(find /media/mvangala/pig_rnseq_data/concat_per_sample_fastq/ -type f -name "2015*_R1.fastq.gz"); do 
	echo "INFO:Processing sample: $left_file" >&2
	echo "-----------------------------------" >&2
	right_file=$(echo $left_file | sed -e "s/_R1/_R2/")
	sample_base_dir=$(basename  $left_file)
	sample_base_dir=$(echo $sample_base_dir | sed -e "s/_.*//");
#	sample_name=$(basename $left_file | sed -e "s/_R1.fastq//");
	sample_name=${sample_base_dir}
#	sample_base_dir="${sample_base_dir}/${sample_name}"
#	trim_reads $left_file $right_file "${ANALYSIS_DIR}/${sample_base_dir}/trimmomatic"
#	align_reads "${ANALYSIS_DIR}/${sample_base_dir}/trimmomatic" "${ANALYSIS_DIR}/${sample_base_dir}/1pass" ${STAR_GENOME_INDEX_DIR}
#	generate_index "${ANALYSIS_DIR}/${sample_base_dir}/2pass_index" $REF_FASTA "${ANALYSIS_DIR}/${sample_base_dir}/1pass/SJ.out.tab" 
#	align_reads "${ANALYSIS_DIR}/${sample_base_dir}/trimmomatic" "${ANALYSIS_DIR}/${sample_base_dir}/2pass" "${ANALYSIS_DIR}/${sample_base_dir}/2pass_index"
	process_picard "/media/mvangala/pig_rnseq_data/STAR/${sample_base_dir}" "${ANALYSIS_DIR}/${sample_base_dir}/picard" $sample_name
	split_n_trim_reads $REF_FASTA  "${ANALYSIS_DIR}/${sample_base_dir}/picard" $sample_name "${ANALYSIS_DIR}/${sample_base_dir}/split_n_trim"
	perform_variant_calling $REF_FASTA "${ANALYSIS_DIR}/${sample_base_dir}/split_n_trim" $sample_name "${ANALYSIS_DIR}/${sample_base_dir}/variant_calling"
done
