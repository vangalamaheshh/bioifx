...
for left_file in $(find /media/mvangala/pig_genome_snp/trimmomatic/20150605-PK15-29-LY1977_S3/ -name "2015*left_paired.fastq*"); do 
	echo "INFO:Processing sample: $left_file" >&2
	echo "-----------------------------------" >&2
	right_file=$(echo $left_file | sed -e "s/left/right/")
	sample_base_dir=$(basename $(dirname $left_file))
...
#	align_reads "${ANALYSIS_DIR}/${sample_base_dir}/trimmomatic" "${ANALYSIS_DIR}/${sample_base_dir}/2pass" "${ANALYSIS_DIR}/${sample_base_dir}/2pass_index"
	process_picard "/media/mvangala/pig_genome_snp/STAR/${sample_base_dir}" "${ANALYSIS_DIR}/${sample_base_dir}/picard" $sample_name
