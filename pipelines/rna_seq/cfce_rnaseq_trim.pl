...
sub write_trimmomatic_commands {
	...
	my $trim_exec = '/zfs/cores/mbcf/mbcf-storage/devel/umv/software/Trimmomatic-0.32/trimmomatic-0.32.jar';
      	my $pe_adapter_file='/zfs/cores/mbcf/mbcf-storage/devel/umv/software/Trimmomatic-0.32/adapters/TruSeq3-PE.fa';
	my $se_adapter_file = '/zfs/cores/mbcf/mbcf-storage/devel/umv/software/Trimmomatic-0.32/adapters/TruSeq3-SE.fa';
	...
	my $job_count = 1;
		...
		if( exists $$trim_info{ 'trim_files' }{ $trim_file_prefix }{ 'PE' } ) {
			...
			my $left_mate_paired_out = $trim_out_dir . '/' . $trim_file_prefix . '.trim.leftmate.paired.fastq';
			my $left_mate_unpaired_out = $trim_out_dir . '/' . $trim_file_prefix . '.trim.leftmate.unpaired.fastq';
			my $right_mate_paired_out = $trim_out_dir . '/' . $trim_file_prefix . '.trim.rightmate.paired.fastq';
			my $right_mate_unpaired_out = $trim_out_dir . '/' . $trim_file_prefix . '.trim.rightmate.unpaired.fastq';
			...
			my $single_mate_out = $trim_out_dir . '/' . $trim_file_prefix . '.trim.singlemate.fastq';
