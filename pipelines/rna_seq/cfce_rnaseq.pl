...
sub get_meta_info {
	...
	my $SE = undef;
	my $source_dir = undef;
	my $trim_single_token = '.trim.singlemate.fastq';
        my $trim_left_token = '.trim.leftmate.paired.fastq';
        my $trim_right_token = '.trim.rightmate.paired.fastq';
		...
		my ( $row_min, $row_max ) = $worksheet->row_range();
			...
			if( $cols[ 0 ] =~ /ilab.*id/i ) {
				...
				if( $sample_type &&  $sample_type =~ /cont/i ) {
					if( $SE ) {
						if( $$options{ 'trim' } ) {
							push @{ $$meta_info{ 'grp_name' }{ $grp }{ 'control' }{ 'files' }{ 'single_mates' } }, $trim_file_prefix . $trim_single_token;
						} else {
						...
						if( $$options{ 'trim' } ) {
							push @{ $$meta_info{ 'grp_name' }{ $grp }{ 'control' }{ 'files' }{ 'left_mates' } }, $trim_file_prefix . $trim_left_token;
                                                        push @{ $$meta_info{ 'grp_name' }{ $grp }{ 'control' }{ 'files' }{ 'right_mates' } }, $trim_file_prefix . $trim_right_token;
						} else {
					...
					if( $SE ) {
						if( $$options{ 'trim' } ) {
							push @{ $$meta_info{ 'grp_name' }{ $grp }{ 'treat' }{ 'files' }{ 'single_mates' } }, $trim_file_prefix . $trim_single_token;
						} else {
						...
						if( $$options{ 'trim' } ) {
							push @{ $$meta_info{ 'grp_name' }{ $grp }{ 'treat' }{ 'files' }{ 'left_mates' } }, $trim_file_prefix . $trim_left_token;
                                                        push @{ $$meta_info{ 'grp_name' }{ $grp }{ 'treat' }{ 'files' }{ 'right_mates' } }, $trim_file_prefix . $trim_left_token;
...
sub get_source_dir {
	my( $run_id ) = @_;
	if( $run_id =~ /(N.50\d)/ ) {
		return "/zfs/cores/mbcf/mbcf-storage/$1/analysis/$run_id/concat_per_sample_fastq";
	}
	else {
...
sub write_rnaseq_commands {
	...
	foreach my $grp( keys %{ $$meta_info{ 'grp_name' } } ) {
		...
		print OFH 'source /zfs/cores/mbcf/mbcf-storage/devel/umv/ROOT/bioifx/pipelines/rna_seq/rnaseq_env.bashrc' . "\n";
		print OFH $command, "\n";
